...
main() {
    ...
    if [ "$(uci -q get firewall.@defaults[0].flow_offloading_hw)" != "1" ]; then
        ...
        if [ -z "$devices" ]; then
            rm -f "$RULES_FILE"
            nft delete table bridge fw4 >/dev/null 2>&1
            logger -t bridge-hw-offload "No bridge ports found for ${BRIDGE}, hardware offload disabled"
            return 1
        fi

        mkdir -p "$RULES_DIR"
        local tmpfile
        tmpfile=$(mktemp "${RULES_DIR}/.30-bridge-offload.XXXXXX") || return 1
        cat > "$tmpfile" <<EOF
